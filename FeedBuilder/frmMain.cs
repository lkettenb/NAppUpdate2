...
namespace FeedBuilder
{
    public partial class frmMain : Form
    {
        ...
        private void InitializeFormSettings()
        {
            ...
            txtIgnoreFiles.Text = Settings.Default.IgnoreFiles;
            ReadFiles();
            UpdateTitle();
        }
        ...
        private void SaveFormSettings()
        {
            ...
            //Settings.Default.IgnoreFiles = string.Empty;
            //foreach (ListViewItem thisItem in lstFiles.Items)
            //{
            //    if (!thisItem.Checked) Settings.Default.IgnoreFiles = string.Concat(Settings.Default.IgnoreFiles, "|", thisItem.Text);
            //}
            Settings.Default.IgnoreFiles = txtIgnoreFiles.Text.Trim();
        }
        ...
        private bool CopyFile(string sourceFile, string destFile)
        {
            ...
            if (!Directory.Exists(d.FullName)) CreateDirectoryPath(d);
        ...
        private void CreateDirectoryPath(DirectoryInfo directory)
        {
            ...
            int retries = 3;
            while (retries > 0 && !directory.Exists)
            {
                directory.Create();
        ...
        private void OpenOutputsFolder()
        {
            ...
            var file = new FileInfo(txtFeedXML.Text.Trim());

            CreateDirectoryPath(file.Directory);
        ...
        private static bool IsIgnorableFile(string fileName)
        {
            foreach (string pattern in Settings.Default.IgnoreFiles.Split(new char[] { '|' }))
            {
                if (Regex.IsMatch(fileName, WildcardToRegex(pattern), RegexOptions.IgnoreCase))
                {
                    return true;
                }
            }
        ...
        private void ApplyIgnoreFiles_Click(object sender, EventArgs e)
        {
            cmdApplyIgnoreFiles.Enabled = false;
            Settings.Default.IgnoreFiles = txtIgnoreFiles.Text;
            ReadFiles();
            cmdApplyIgnoreFiles.Enabled = true;
        }
    }
}
